...
perf_record: bin/stripped-1brc bin/symbols-1brc
	sudo perf record -e cpu-cycles:PH -F 4096 --call-graph dwarf bin/stripped-1brc data/100M.txt 1 > 100M-test.txt
...
bin/pgo-1brc: $(wildcard src/*)
	mkdir -p bin/
	$(CXX) $(CXXFLAGS) -DNDEBUG -static -fprofile-instr-generate -O3 src/main.cc -o $@
