...
static int AES_KeySetup_Enc_AESNI(uint8_t rk8[] /*16*(Nr + 1)*/,
  ...
  return (keyBits == 128)   ? 10
         : (keyBits == 192) ? 12
         : (keyBits == 256) ? 14
                            : 0;
}

static int AES_KeySetup_Dec_AESNI(uint8_t rk8[] /*16*(Nr + 1)*/,
                                  const uint8_t cipherKey[], int keyBits) {
  __m128i *round_keys = (__m128i *)rk8;
...
template <int Nr>
static inline void AES_Encrypt_AESNI(const uint8_t rk8[] /*16*(Nr + 1)*/,
...
template <int Nr>
static inline void AES_Decrypt_AESNI(const uint8_t rk8[] /*16*(Nr + 1)*/,
...
static inline void AES_EncryptRound_AESNI(const uint8_t rk8[],
                                          uint8_t block[16]) {
